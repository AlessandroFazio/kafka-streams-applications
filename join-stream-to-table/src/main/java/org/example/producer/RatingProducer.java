...
public class RatingProducer {
    ...
    private static String TOPIC = "ratings";
    ...
    private KafkaProducer<String, Rating> producer;
    private boolean closed = false;

    public RatingProducer(KafkaProducer<String, Rating> producer) {
        this.producer = producer;
    }

    public void produceRecords(long[] uuidPool) {
        try {
            int i = 0;
            while(!closed) {
                int index = i++ % uuidPool.length;
                Rating event = Rating.newBuilder()
                        .setId(uuidPool[index])
                        .setRating(RANDOM.nextDouble(4.0, 10.0))
                        .build();

                ProducerRecord<String, Rating> record = new ProducerRecord<>(
                        TOPIC, "", event);
